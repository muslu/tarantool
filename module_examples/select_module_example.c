...
cfg(lua_State *L)
{
	...
	if (pthread_create(&context.thread, NULL, module_thread_f, NULL) < 0) {
		lua_pushstring(L, "error: can't create thread");
		lua_error(L);
	}
	return 0;
}
...
LUALIB_API int
luaopen_select_module_example(lua_State *L)
{
	const char *error;
